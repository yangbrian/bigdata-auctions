...
router.get('/customer/:name', auth, function (req, res) {
    ...
    db.query(
        'SELECT Person.FirstName, Person.LastName, Item.Name AS ItemName, Sales.Price, Sales.Date ' +
        'FROM Sales ' +
        'INNER JOIN Item ' +
        'ON Sales.ItemID = Item.ItemID ' +
        'INNER JOIN Person ' +
        'ON Sales.BuyerID = Person.SSN ' +
        'Where Person.LastName = ? ', [req.params.name], function (err, rows) {
...
/**
 * Revenue generated by customer last name
 */
router.get('/revenue/customer/:name', auth, function (req, res) {

    db.query(
        'SELECT Person.FirstName, Person.LastName, Item.Name AS ItemName, sum(Sales.Price) as Revenue ' +
        'FROM Sales ' +
        'INNER JOIN Item ' +
        'ON Sales.ItemID = Item.ItemID ' +
        'INNER JOIN Person ' +
        ...
        'GROUP BY Person.LastName', [req.params.name], function (err, rows) {
            res.setHeader('content-type', 'application/json');
            return res.send(JSON.stringify(rows));
        }
    );
